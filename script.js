const ASSETS = {
    ...
    IMAGE: {
      ...
      SKY: {
        src: ['./assets/bg/day.png',
                './assets/bg/early.png',
                './assets/bg/midnight.png',
                './assets/bg/late-night.png',
            ]
      }
    },
...
Number.prototype.pad = function(numToPad, char = 0) {
    let n = Math.abs(this)
    let numZeros = Math.max(0, numToPad - Math.floor(n).toString().length)
    let zeroString = Math.pow(10, numZeros).toString().substr(1).replace(0, char)
    return zeroString + n
}
...
function genMap() {
    ...
    for (var i = 0; i < mapLength; i += getRand(0, 50)) {
        ...
        if (Math.random() > .9) Object.assign(section, {curve: _ => randCurve,
                                                        height: _ => randHeight})
        else if (Math.random() > .8) Object.assign(section, {curve: _ => 0,
...
addEventListener(`keyup`, function(e) {
    ...
    if (e.code === 'KeyM') {
        ...
        audio.volumn = (audio.volume === 0) ? 1 : 0
        return
    }
...
function update(step) {
    ...
    if(KEYS.ArrowRight) {
        hero.style.backgroundPosition = '-900px 0'
        playerX += .007 * step * speed
    }
    else if(KEYS.ArrowLeft) {
        hero.style.backgroundPosition = '-180px 0'
        playerX -= .007 * step * speed
    }
    else hero.style.backgroundPosition = '-540px 0'
    ...
    else if(KEYS.ArrowDown) speed = accelerate(speed, braking, step)
    ...
    if(!inGame) {
        speed = accelerate(speed, braking, step)
        speed = speed.clamp(0, maxSpeed)
...
function updateBackground() {
    bgIndex = (bgIndex + 1) % ASSETS.IMAGE.SKY.src.length;
    
    cloudNext.style.backgroundImage = `url(${ASSETS.IMAGE.SKY.src[bgIndex]})`;

    cloudNext.style.opacity = 1;
...
function init() {
    cloud.style.backgroundImage = `url(${ASSETS.IMAGE.SKY.src[bgIndex]})`;
